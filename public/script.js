/* ========= Configuration ========= */
const API_BASE_URL = "http://localhost:5000/api"; // change to production base when deployed
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (qs("steps") || qs("heartRate")) {
    ...
    // Auto-refresh vitals every 5 minutes if Google Fit is connected
    setInterval(async () => {
      try {
        const statusResponse = await fetch(`${API_BASE_URL}/googlefit/status`);
        if (statusResponse.ok) {
          const status = await statusResponse.json();
          if (status.connected) {
            console.log("🔄 Auto-refreshing vitals from Google Fit...");
            await loadVitals();
          }
        }
      } catch (err) {
...
/* ========= Vitals (load from Google Fit if connected, otherwise vitals.json) ========= */
async function loadVitals() {
  ...
  try {
    ...
    if (statusResponse.ok) {
      ...
      if (status.connected) {
        ...
        const fitResponse = await fetch(`${API_BASE_URL}/googlefit/vitals`);
        if (fitResponse.ok) {
          data = await fitResponse.json();
          useGoogleFit = true;
          console.log("✅ Loaded vitals from Google Fit");
        } else {
          console.warn("Google Fit connected but failed to fetch data, falling back to local");
        }
      }
    }
  } catch (e) {
  ...
  // If Google Fit not available or failed, load from vitals.json
  if (!data || !useGoogleFit) {
    try {
      const response = await fetch("vitals.json"); // Served statically by backend
      data = await response.json();
      console.log("📊 Loaded vitals from local file");
    } catch (err) {
      console.error("Failed to load vitals.json:", err);
      // Use default data structure
      data = {
        labels: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
        steps: [5000, 5000, 5000, 5000, 5000, 5000, 5000],
        heartRate: [72, 72, 72, 72, 72, 72, 72],
        sleepHours: [7.0, 7.0, 7.0, 7.0, 7.0, 7.0, 7.0],
        bp: ["120/80", "120/80", "120/80", "120/80", "120/80", "120/80", "120/80"],
        energy: [60, 60, 60, 60, 60, 60, 60],
        stressLevel: [50, 50, 50, 50, 50, 50, 50],
        temperature: [36.6, 36.6, 36.6, 36.6, 36.6, 36.6, 36.6]
      };
    }
  }

  // Merge with localStorage data (manual entries take priority over Google Fit)
  try {
    ...
    // First, add all data from Google Fit or vitals.json
    if (data && data.labels) {
      data.labels.forEach((label, index) => {
        vitalsByDate.set(label, {
          steps: data.steps[index] || 0,
          heartRate: data.heartRate[index] || 72,
          sleepHours: data.sleepHours[index] || 7,
          bp: data.bp[index] || "120/80",
          energy: data.energy[index] || 60,
          stressLevel: data.stressLevel[index] || 50,
          temperature: data.temperature[index] || 36.6
        });
      });
    }

    // Then merge manual entries (these take priority over Google Fit data)
